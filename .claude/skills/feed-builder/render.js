...
const CFG = JSON.parse(fs.readFileSync(path.join(__dirname, 'feed.config.json'), 'utf8'));

const ACCENT = process.env.FEED_ACCENT || CFG.accent || '#e63f18';
...
// דירוג הישימות מוצג במילים ולא בנקודות, כדי שלא יידרש מקרא
const RANKS = {
  3: { label: CFG.labels['3'], numeral: ACCENT, chipBg: INK, chipText: '#ffffff', chipBorder: INK },
  2: { label: CFG.labels['2'], numeral: INK, chipBg: '', chipText: INK, chipBorder: INK },
  1: { label: CFG.labels['1'], numeral: FADE, chipBg: '', chipText: MUTED, chipBorder: FADE }
};

// תקרות ורצפות. הניסיון הראה שהנחיה בפרוזה לא מחזיקה מול הלחץ
// להגיע ליעד הכמות, ולכן הבדיקות כאן חוסמות רינדור.
const MAX_PER_SOURCE = CFG.maxPerSource || 2;
const MAX_PER_URL = CFG.maxPerUrl || 2;
const DOER_TYPES = CFG.doerTypes || [];
...
function checkMix(items) {
  if (!DOER_TYPES.length || items.length < MIN_ITEMS_FOR_MIX) return;
  ...
  if (doers < required) {
    throw new Error('הפיד כולו הודעות ספקים ולכן לא רונדר:\n' +
      ...
      ' מתוך ' + items.length + '\n' +
      'חפש מה אנשים גילו שאפשר לעשות, לא רק מה שחברות הכריזו.\n' +
      'פיד שכולו "חברה X הכריזה" הוא לא הפיד הזה.');
  }
}
...
function howToBlock(item, quiet) {
  // הפריט החלש מקבל טיפול שקט, בלי הקופסה הכבדה
  if (quiet) {
    return spacer(12) +
  ...
  return spacer(20) +
    ...
    table('width="100%"',
      ...
      ';letter-spacing:1.6px;">' + esc(CFG.ui.howTo) + '</div>' +
...
function promptBlock(text) {
  ...
  return spacer(12) +
    ...
    table('width="100%"',
      ...
      ';letter-spacing:1.6px;">' + esc(CFG.ui.prompt) + '</div>' +
...
function angleBlock(text) {
  return spacer(14) +
    ...
    table('width="100%"',
      ...
      '<span style="font-weight:bold;">' + esc(CFG.ui.angle) + ' - </span>' + nl2br(text) +
      '</td></tr>') +
    '</td></tr>';
}
...
function renderItem(item, index) {
  ...
  if (item.how_to) rows.push(howToBlock(item, quiet));
  ...
  if (item.url) {
    ...
    rows.push('<tr><td><a href="' + esc(item.url) + '" style="font-family:' + FONT +
      ...
      ';letter-spacing:0.6px;text-decoration:none;">' + esc(CFG.ui.readMore) + ' ' + ARROW + '</a></td></tr>');
  }
...
function masthead(feed) {
  ...
  const line = String(CFG.ui.tagline || '').replace('{n}', feed.items.length);

  return '<tr><td bgcolor="' + INK + '" style="background-color:' + INK + ';padding:34px 40px 30px 40px;">' +
    table('width="100%"',
      '<tr>' +
      '<td valign="middle" style="font-family:' + FONT +
      ';font-size:40px;font-weight:bold;color:#ffffff;letter-spacing:-1.2px;line-height:40px;">' +
      esc(CFG.wordmark) + '</td>' +
      '<td valign="middle" align="' + (DIR === 'rtl' ? 'left' : 'right') + '">' +
      table('',
...
function renderEmail(feed) {
  ...
  const inner = table('width="600" dir="' + DIR + '" bgcolor="#ffffff"',
    masthead(feed) + spacer(34) + body.join('') + spacer(34) + footer());

  return '<!DOCTYPE html>\n' +
    '<html dir="' + DIR + '" lang="' + esc(CFG.lang || 'he') + '"><head>' +
    '<meta charset="utf-8">' +
    '<meta name="viewport" content="width=device-width,initial-scale=1">' +
    '<title>' + esc(CFG.title + ' - ' + feed.date) + '</title>' +
    '</head>' +
    '<body dir="' + DIR + '" style="margin:0;padding:0;background-color:' + PAGE + ';">' +
    table('width="100%" dir="' + DIR + '" bgcolor="' + PAGE + '"',
...
function localDate() {
  try {
    return new Intl.DateTimeFormat(CFG.locale || 'he-IL', {
...
function normalize(data) {
  ...
  checkMix(items);
  return { date: data.date || localDate(), items: items };
}
...
try {
  ...
  console.log('SUBJECT: ' + CFG.title + ' - ' + feed.date);
} catch (err) {
